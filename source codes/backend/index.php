...
echo '<p>Current map selected : ' . $_SESSION['mapid'] . '</p>';
?>
    ...
    <header id="header">
        ...
        <nav id="nav" class="clearfix">
            <ul>
                ...
                <a href="http://cloudangel.16mb.com/notification/firebase.html" target="_blank">Firebase Notification</a>
                <li id="bola" class="idle"><a href="#">Add Node</a></li>
            </ul>
        </nav>
        <div id="coords"></div>
        <div id="debug"></div>
...
<h2 class="title hide">Click on the image location to add node</h2>
...
<div class="tab" id="tab">
  ...
  <button class="tablinks" onclick="openCity(event, 'node_list')">Node</button>
</div>
    ...
    if($_SESSION['mapid'] != null && !empty($_SESSION['mapid']))
    {
      ...
      if(mysqli_num_rows($sql_area) > 0)   //if query runs
      {
        ...
        while ($row = mysqli_fetch_array($sql_area)){
          ...
          echo '<br/> Area Name : <button id="tap_area" name="tap_area" class="tap_area idle" value="'.$coords.'">' .$name . '</button>';
          echo '<button id="update_area" name="update_area" class="update_area" value="'.$coords.'">Update</button>';
          echo '<button name="edited_coor" class="edited_coor" value="">Save</button><br/>';
          echo '<br/> Edited : <input type="text" name="update_coordinate" class="update_coordinate">';
    ...
    if($_SESSION['mapid'] != null && !empty($_SESSION['mapid']))
    {
      ...
      if($sql_path){
          while ($row = mysqli_fetch_array($sql_path)){
            ...
            echo '<br/> Path Name : <button id="tap_path" name="tap_path " class="tap_path idle" value="'.$coords.'">' .$name. '</button><br/>';
            echo 'Coordinate : ' . $coords;
            echo '<br/>';
...
<div id="node_list" class="tabcontent2">
    <div id="path_content">
        NODES
        <?php
        // Check connection
        ...
        {
            $getmapid = $_SESSION['mapid'];
            $sql_path = mysqli_query($dbconfig, "SELECT node.id, pathid, node.name, node.coords, node.link, node.info from node join area
                                                        ON node.areaid = area.id where area.mapid=$getmapid");
            ...
            if($sql_path){
                while ($row = mysqli_fetch_array($sql_path)){
                    ...
                    //echo '<button name="node" class="hide node">n</button>';
                    echo '<br/> Node Name : <button name="tap_node" class="tap_node idle" data-value="'.$id.'" value="'.$coords.'">' .$name. '</button><br/>';
                    echo 'Coordinate : ' . $coords;
                    echo '<br/>';
    ...
    <div id="map_list">
      <div id="map_list_content">
        ...
        while ($row = mysqli_fetch_array($sql_map)){
          ...
          echo  '<br/>'.$map_count.'.'.$name.'<br/><input type="submit" onclick="setCurrentMap('.$map_id.')" class="mapmap" value=" ' . $image . '"/><br/>';
        }
...
<div class="hide" id="workshop-div"><h3 style="color:brown">Node Details</h3>
    <form action="addnode.php" method="post" enctype="multipart/form-data">
            <div class="list-group-item">
                ...
                if(mysqli_num_rows($sql_path) > 0) {
                    echo 'Path Name : <select name="path_id">
                    <option value="" selected disabled>Path </option> \';';
                    while ($row = mysqli_fetch_array($sql_path)) {
                        $path_name = $row['name'];
                        $path_id = $row['id'];
                        echo ' <option value="' . $path_id . '">' . $path_name . '</option>';
                    }
                    echo '</select><br/>';
                }
                ?>
                Name : <input type="text" name="name" placeholder="Node Name" class="form-control no-border" required><br/>
                Coordinate : <input type="text" name="coordinate" id="node_coordinate" placeholder="Coordinate" class="form-control no-border" required readonly><br/>
                <?php
                // Check connection
                if ($dbconfig->connect_error) {
                    die("Connection failed: " . $dbconfig->connect_error);
                ...
                if(mysqli_num_rows($sql_area) > 0) {
                    echo 'Area name : <select name="area_id">
                ...
                ?>
                Link To : <input type="text" name="link" placeholder="Link" class="form-control no-border" ><br/>
                Information : <input type="text" name="info" placeholder="Info" class="form-control no-border" ><br/>
                <input type="submit" name="submit_workshop" id="submit_workshop" class="btn btn-md btn-warning btn-block"></button>
            </div>
    </form>
</div>
...
<script type="text/javascript" src="addnode.js"></script>
<script>
...
function saveCoordinate(str) {
    ...
    if (str == "") {
        ...
        xmlhttp.open("GET","updateCoordinate.php?q="+str,true);
        xmlhttp.send();
        //location.reload();
    }
}
