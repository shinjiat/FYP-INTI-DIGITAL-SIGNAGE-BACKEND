...
var summerHtmlImageMapCreator = (function() {
    ...
    var app = (function() {
        ...
        function onSvgMousedown(e) {
            if (state.appMode === 'editing') {        //editing mode
                if (e.target.parentNode.tagName === 'g') {
                    info.unload();
                    state.selectedArea = e.target.parentNode.obj;

                    app.deselectAll();
                    state.selectedArea.select();      //editing mode
                    ...
                    if (e.target.classList.contains('helper')) {
                    ...
                    } else if (e.target.tagName === 'rect' || e.target.tagName === 'circle' || e.target.tagName === 'polygon') {    //drawing mode
                        state.editType = 'move';
                      ...
                      else if (e.target.tagName === 'bullet'){
                        state.editType = 'move';
    ...
    Polygon.prototype.toString = function() {
        return this._coords.points.map(function(item) {
    ...
    //THIS IS FOR BULLETS
    function Bullet(coords, attributes) {
        Area.call(this, 'bullet', coords, attributes);
         ...
         */
        this._coords = {
            points : coords.points || [{x: 0, y: 0}],         //coords.points are only for polygon shape
            isOpened : coords.isOpened || false
        };

        this._el = document.createElementNS(
            Area.SVG_NS,
            this._coords.isOpened ? 'polyline' : 'bullet'   //edited to bullet
        );
        this._groupEl.appendChild(this._el);
        ...
        for (var i = 0, c = this._coords.points.length; i < c; i++) {   //HELPER IS TO SHOW THE SMALL NODE
          console.log(this._coords.points.length);      //ALWAYS 1
            this._helpers.points.push(
                (new Helper(
                    this._groupEl,
                    this._coords.points[i].x,
    ...
    Bullet.prototype.close = function() {
        var polyline = this._el;
        this._el = document.createElementNS(Area.SVG_NS, 'bullet');   //edited
      //  this._groupEl.replaceChild(this._el, polyline);
     ...
     * @returns {Polygon} - this area
     */
    Bullet.prototype.setSVGCoords = function(coords) {
        var polygonPointsAttrValue = coords.points.reduce(function(previousValue, currentItem) {
            return previousValue + currentItem.x + ' ' + currentItem.y + ' ';
        }, '');
     ...
     * @returns {Polygon} - this area
     */
    Bullet.prototype.setCoords = function(coords) {
        this._coords.points = coords.points;
     ...
     * @returns {Polygon} - this area
     */
    Bullet.prototype.addPoint = function(x, y) {
        if (!this._coords.isOpened) {
            throw new Error('This polygon is closed!');
        }

        var helper = new Helper(this._groupEl, x, y);    //remove 'movePoint'
        helper.setId(this._helpers.points.length);
     ...
     * @returns {Object} - calculated coordinates
     */
    Bullet.prototype.dynamicDraw = function(x, y, isRightAngle) {
        var temp_coords = {
            points: [].concat(this._coords.points)
        };

        if (isRightAngle) {
            var rightPointCoords = Bullet.getRightAngleLineLastPointCoords(
                this._coords, {x: x, y: y}
            );
            x = rightPointCoords.x;
            y = rightPointCoords.y;
     ...
     * @params e {MouseEvent} - mousemove event
     */
    Bullet.prototype.onProcessDrawing = function(e) {
        var coords = utils.getRightCoords(e.pageX, e.pageY);
     ...
     * @params e {MouseEvent} - click event
     */
    Bullet.prototype.onAddPointDrawing = function(e) {
        var newPointCoords = utils.getRightCoords(e.pageX, e.pageY);

        if (e.shiftKey) {
            newPointCoords = Bullet.getRightAngleLineLastPointCoords(this._coords, newPointCoords);
        }
     ...
     * @params e {KeyboardEvent|MouseEvent} - click or keydown event
     */
    Bullet.prototype.onStopDrawing = function(e) {
        if (e.type == 'click' || (e.type == 'keydown' && e.keyCode == 13)) {
            if (Bullet.testCoords(this._coords)) {
                this.close();
     ...
     * @returns {Object} - Object with changed parameters of area
     */
    Bullet.prototype.edit = function(editingType, dx, dy) {
        var tempParams = Object.create(this._coords);
     ...
     * @params e {MouseEvent} - mousemove event
     */
    Bullet.prototype.onProcessEditing = function(e) {
        var editType = app.getEditType();
     ...
     * @returns {string}
     */
    Bullet.prototype.toHTMLMapElementString = function() {
        var str = this._coords.points.map(function(item) {
            return item.x + ', ' + item.y;
        }).join(', ');
     ...
     * @returns {Object} - coordinates of point
     */
    Bullet.prototype.getCoordsForDisplayingInfo = function() {
        return {
            x : this._coords.points[0].x,
            y : this._coords.points[0].y
        };
     ...
     * @returns {boolean}
     */
    Bullet.testCoords = function(coords) {
        return coords.points.length >= 3;
    };
     ...
     * @returns {boolean}
     */
    Bullet.testHTMLCoords = function(coords) {
        return coords.length >= 6 && coords.length % 2 === 0;
    };
     ...
     * @returns {Object} - object with calculated points
     */
    Bullet.getCoordsFromHTMLArray = function(htmlCoordsArray) {
        if (!Bullet.testHTMLCoords(htmlCoordsArray)) {
            throw new Error('This html-coordinates is not valid for polygon');
        }
     ...
     * @returns {Object} - Right coordinates for last point
     */
    Bullet.getRightAngleLineLastPointCoords = function(originalCoords, newPointCoords) {
        var TANGENS = {
                DEG_22 : 0.414,
                DEG_67 : 2.414
            },
     ...
     * @static
     * @param firstPointCoords {Object} - coords for first point of polyline
     * @returns {Bullet} - created polygon
     */
    Bullet.createAndStartDrawing = function(firstPointCoords) {
        var newArea = new Bullet({
            points : [firstPointCoords],
            isOpened : true
        });
    ...
    var code = (function(){
        var block = utils.id('code'),
            ...
            coordinate = utils.id('coordinate'),
            close_button = block.querySelector('.close_button');
        ...
        return {
            print: function() {
                ...
                coordinate.value = app.getHTMLCode(true);
                utils.show(block);
            },
            hide: function() {
                utils.hide(block);
    ...
    /* For html code of created map */   //ADD & edited
    var path_code = (function(){
        var block = utils.id('path_code'),
            path_content = utils.id('path_content'),
            path_coordinate = utils.id('path_coordinate'),
            close_button = block.querySelector('.close_button');
        ...
        return {
            print: function() {
                //content.innerHTML = app.getHTMLCode(true);
                path_coordinate.value = app.getHTMLCode(true);
                utils.show(block);
            },
            hide: function() {
                utils.hide(block);
    ...
    /*Load areas from area box*/      //ADDED & EDITED
    var list_area = (function(){
      var form = utils.id('list');
          list_content = utils.id('list_content');
          var tap_area = document.getElementsByClassName("tap_area");
          var tap_path = document.getElementsByClassName("tap_path");



          function load_area(e){
              if($(this).hasClass('idle')) {

                  $(this).removeClass('idle');
                  $(this).addClass('pressed');
                  if (Area.createAreasFromHTMLOfMap(this.value)) {
                      console.log("area");
                  }
              }
              else if($(this).hasClass('pressed')){           //SUPPOSED TO REMOVE AREA
                  $(this).addClass('idle');
              }
          ...
          var i;
          for(i=0; i < tap_area.length; i++){
            tap_area[i].addEventListener('click', load_area, false);
            tap_area[i].value = '<area shape="poly" coords=" ' + tap_area[i].value + '" />';
          }

          for(i=0; i < tap_path.length; i++){
            tap_path[i].addEventListener('click', load_area, false);
            tap_path[i].value = '<area shape="poly" coords=" ' + tap_path[i].value + '" />';
          }
    ...
    var from_html_form = (function() {
        ...
        return {
            show : function() {
                alert("Shape code : \n Polygon = poly \n Circle = circle \n Rectangle = rect")
                code_input.value = '<area shape="poly" coords="" />';
                utils.show(form);
            },
            hide : hide
        };
    ...
    var get_image = (function() {
        var block = utils.id('get_image_wrapper'),
            ...
            //test on creating node
            button = utils.id('button'),
      //      canvas = utils.id('canvas'),
            ...
            //map list
            map_list = utils.id('map_list');
            var mapmap = document.getElementsByClassName('mapmap');
            utils.hide(list);
            utils.hide(path_list);
            utils.show(map_list); //show existing map
  ...
  /*Retrieve IMAGE MAP ID from database */
        function onMapClick(e) {
            url.value = "http://cloudangel.16mb.com" + this.value;
            url.value = url.value.replace(/\s+/g, '');
          //  alert(url.value);

    //        utils.hide(canvas); //hide canvas
        ...
        var i;
        for(i=0; i < mapmap.length; i++){
            mapmap[i].addEventListener('click', onMapClick, false);
        }
    ...
    var buttons = (function() {
        var all = utils.id('nav').getElementsByTagName('li'),
            ...
            bola = utils.id('bola'),
            //path = utils.id('path');
            save_path = utils.id('save_path'),
            update_coordinate = document.getElementsByClassName('update_coordinate'),
            update_area = document.getElementsByClassName('update_area');
/*
        function onUpdateAreaButtonClick(e){
            update_coordinate[update_area.indexOf(this)].value = app.getHTMLCode(true);
        }*/

        for(var i=0; i < update_area.length; i++){
            (function(index){
                update_area[i].onclick = function(){
                    console.log(index);
                    update_coordinate[index].value = app.getHTMLCode(true);
                }
            })(i);
           // update_area[i].value = '<area shape="poly" coords=" ' + update_area[i].value + '" />';
            //update_coordinate[i].value = update_area[i].value;
        }
        ...
        function selectOne(button) {
            ...
            //console.log('selected ' + Area.CLASS_NAMES.SELECTED);
        }
        ...
        function onBolaButtonClick(e){
            console.log(this.id);

            /*
            app.setMode('drawing')    //set mode to drawing
            .setDrawClass()         //remove edit, set draw
            .setShape(this.id)      //set shape
            .deselectAll()          //
            .hidePreview();
         info.unload();
         selectOne(this);
        ...
        function onSavePathButtonClick(e){
            info.unload();
            path_code.print();
        ...
        save_path.addEventListener('click', onSavePathButtonClick, false);
        ...
        bola.addEventListener('click', onBolaButtonClick, false);
       // path.addEventListener('click', onShapeButtonClick, false);
    })();
